...
export async function promoteToAdmin(userId: string, userEmail: string, displayName: string): Promise<{ success: boolean, error?: string }> {
    ...
    // For now, only the super admin can promote others
    if (promoter.email !== 'admin@xavef.com') {
        return { success: false, error: "You do not have permission to promote users." };
    }
