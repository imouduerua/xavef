...
export function AvailableGroupsList() {
  ...
  const groupsQuery = useMemoFirebase(() => firestore ? query(
  ...
  if (!availableGroups || availableGroups.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-muted-foreground/30 bg-muted/20 p-12 text-center">
        ...
        <h3 className="mt-4 text-lg font-semibold">No New Groups Available</h3>
        <p className="mb-4 mt-2 text-sm text-muted-foreground">
          There are no new savings groups available for you to join right now. Why not create one?
        </p>
      </div>
    );
  }
